...
/* USER CODE BEGIN 1 */
#define TX_THREAD_USER_EXTENSION void *p_user_extension;int bsd_errno;

#if defined(HOOKS_ENABLED)
typedef  void                      (*OS_APP_HOOK_VOIDPTR_INT)(void *p_task, int arg);
typedef  void                      (*OS_APP_HOOK_VOIDPTR)(void *p_task);
typedef  void                      (*OS_APP_HOOK_VOID)(void );
extern   OS_APP_HOOK_VOIDPTR           OS_TaskCreatedHookPtr;
extern   OS_APP_HOOK_VOIDPTR           OS_TaskDeletedHookPtr;
extern   OS_APP_HOOK_VOIDPTR_INT       OS_TaskCtxSwHookPtr;
#define TX_THREAD_CREATE_INTERNAL_EXTENSION(t)	\
		if(0 != OS_TaskCreatedHookPtr)			\
		{										\
			OS_TaskCreatedHookPtr((t));			\
		}

#define TX_USER_THREAD_DELETE_PORT_COMPLETION(t)		\
		if(0 != OS_TaskDeletedHookPtr)			\
		{										\
			OS_TaskDeletedHookPtr(t);			\
		}
